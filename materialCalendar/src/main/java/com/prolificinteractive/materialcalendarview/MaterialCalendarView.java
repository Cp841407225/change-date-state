...
public class MaterialCalendarView extends ViewGroup {
    ...
    public MaterialCalendarView(Context context, AttributeSet attrs) {
        ...
        try {
            ...
            setTitleColor(a.getColor(R.styleable.MaterialCalendarView_mcv_titleColor, Color.WHITE));

            int monthBgColor = a.getColor(R.styleable.MaterialCalendarView_mcv_monthPagerBg, Color.TRANSPARENT);
            if (monthBgColor != 0 && Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN) {
                pager.setBackgroundColor(monthBgColor);
            }
    ...
    protected void onMeasure(final int widthMeasureSpec, final int heightMeasureSpec) {
        ...
        setMeasuredDimension(
                ...
                clampSize(measuredHeight, heightMeasureSpec) - measureTileSize / 2
        );
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            // 解决日历title左右两边的1px留白问题
            if (i == 0) {
                child.layout(childLeft - 1, childTop, childLeft + width + 1, childTop + height);
            } else {
    ...
    public void updateDayStatus(List<DayStatus> status) {
        adapter.setDayStatus(status);
    }
}
