...
class MonthView extends ViewGroup implements View.OnClickListener {
    ...
    public void setDayStatus(List<DayStatus> dayStatus) {
        for (DayView dayView : monthDayViews) {
            String tagDay = dayView.getDate().toString();
            for (DayStatus ds : dayStatus) {
                if (ds.date.equals(tagDay)) {
                    dayView.setDayStatus(ds);
                    dayView.setTextAppearance(getContext(),R.style.TextAppearance_MaterialCalendarWidget_Date_Course);
                }
            }
        }
        updateUi();
